...
function parseUplink(device, payload) {
    ...
    if (decoded.n02 != null) {
        ...
        if (sensor2 != null)
            sensor2.updatePpmConcentrationSensorStatus(decoded.n02);
    };
}
...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        if (bytes[2] === 0x00) { // version report
            ...
            return decoded;
        } else if ((bytes[0] === 0x01) && (bytes[1] === 0x57) && (bytes[2] === 0x06)) { // device type 57 (R718PA5)
            decoded.devicetype = "R718PA5";
            decoded.battery = bytes[3] / 10;
            decoded.n02 = ((bytes[4] << 8) | bytes[5]) / 10;
            return decoded;
        }
